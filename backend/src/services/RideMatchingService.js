...
class RideMatchingService {
  ...
  async findNearbyDrivers(lat, lng, radiusKm = 5, limit = 10) {
    try {
      // Haversine-based distance query using PostgreSQL
      const drivers = await Driver.findAll({
        ...
        include: [
          ...
          {
            association: 'vehicle',
            attributes: ['registration_number', 'vehicle_model', 'color', 'vehicle_image_url'],
          },
        ],
        limit,
        order: [['total_rides', 'DESC']], // Prefer experienced drivers
      });
   ...
   * @param {Object} ride - Ride model instance
   * @param {Function} onDriverFound - Callback when driver accepts
   * @param {Function} onNoDrivers - Callback when no drivers found
   */
  async startSearch(ride, onDriverFound, onNoDrivers) {
    ...
    // Send request to first driver
    await this._sendRequestToNextDriver(rideId, onDriverFound, onNoDrivers);

    // Set timeout for entire search (60 seconds)
  ...
  /**
   * Send ride request to the next available driver
   */
  async _sendRequestToNextDriver(rideId, onDriverFound, onNoDrivers) {
    ...
    // Emit socket event to driver (handled by socket layer)
    // Return driver info so socket layer can emit
  ...
  /**
   * Handle no drivers found or all rejected
   */
  ...
  /**
   * Cancel active search for a ride
   */
  cancelSearch(rideId) {
    this._cleanup(rideId);
  }
