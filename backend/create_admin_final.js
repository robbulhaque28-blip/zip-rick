...
async function run() {
  ...
  if (colNames.includes('password_hash')) {
    sql = "INSERT INTO admin_users (id, user_id, email, password_hash, role, is_active, created_at, updated_at) VALUES ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'admin@zip-rick.com', '" + hash + "', 'super_admin', 1, datetime('now'), datetime('now'))";
  } else if (colNames.includes('password')) {
    sql = "INSERT INTO admin_users (id, user_id, email, password, role, status, created_at, updated_at) VALUES ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'admin@zip-rick.com', '" + hash + "', 'super_admin', 'active', datetime('now'), datetime('now'))";
  }
  
  await sequelize.query(sql);
  console.log('Admin created');
  
  const [admins] = await sequelize.query("SELECT * FROM admin_users JOIN users ON users.id = admin_users.user_id WHERE admin_users.email='admin@zip-rick.com'");
