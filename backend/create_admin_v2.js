...
async function run() {
  ...
  await sequelize.query(
    "INSERT INTO admin_users (id, user_id, username, email, password, role, status, created_at, updated_at) VALUES ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'admin', 'admin@vybe.com', '" + hash + "', 'super_admin', 'active', datetime('now'), datetime('now'))"
  );
  
  console.log('Admin created with current schema');
  
  // Verify
  const [admins] = await sequelize.query("SELECT * FROM admin_users WHERE email='admin@vybe.com'");
